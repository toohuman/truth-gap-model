...
def main():
    ...
    for test in range(tests):
        ...
        # Reusable vector for error values of population
        error_values = np.array([0.0 for x in range(arguments.agents)])

        # Pre-loop results based on agent initialisation.
        for a, agent in enumerate(network.nodes):
            error_values[a] = results.error(agent_type, agent.belief, true_state)

        error_results[0][test] = [
            np.average(error_values),
            np.std(error_values),
            np.min(error_values),
            np.max(error_values)
        ]
        ...
        for iteration in range(1, iteration_limit + 1):
            ...
            if main_loop(
                ...
                for a, agent in enumerate(network.nodes):
                    error = results.error(agent_type, agent.belief, true_state)
                    error_values[a] = error
                    if iteration == iteration_limit:
                        steady_state_results[test][a] = error

                error_results[iteration][test] = [
                    np.average(error_values),
                    np.std(error_values),
                    np.min(error_values),
                    np.max(error_values)
                ]
                # print(error_results[iteration][test][0])
    ...
    # Write error results to pickle file
    if arguments.agents in trajectory_populations:
        with lzma.open(directory + "error" + '_' + '_'.join(file_name_params) + '.pkl.xz', 'wb') as file:
    ...
    with lzma.open(directory + "steady_state_error" + '_' + '_'.join(file_name_params) + '.pkl.xz', 'wb') as file:
